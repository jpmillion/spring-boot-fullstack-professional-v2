...
import {
    ...
    Menu,
    Breadcrumb,
    Table, Spin, Empty, Button, Badge, Tag, Popconfirm, Radio
} from 'antd';
import {
    DesktopOutlined,
    PieChartOutlined,
...
const removeStudent = (studentId, callback) => {
    deleteStudent(studentId)
        .then(() => {
            ...
            callback()
        }).catch(e => e.json().then(e => errorNotification("Student id does not exist", `${e.message} (${e.status}) (${e.error})`)))
}
...
function App() {
    ...
    const fetchStudents = () =>
        getAllStudents()
            ...
            .catch(e => errorNotification("Unable to load", `${e.message} (${e.status}) (${e.error})`))
